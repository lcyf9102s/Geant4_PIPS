...
**PIPS dead-layer energy calibration:** the combined effect of Fe self-absorption (Am-241 source encapsulation), the Si dead layer, and the region-cut boundary above shifts the raw PIPS peak substantially below the true alpha energy (Am-241: 5.486 MeV true vs. 4.8633 MeV raw measured with `run3.mac`, ~11% low). `csv_to_dat()` in `g4decay.cc` applies a linear calibration (`PIPS_calib_slope`/`PIPS_calib_offset`, `E_true_calib = slope*E_meas + offset`) before binning, fit from two known lines (Am-241 and Po-218, both via `run3.mac`) and cross-validated against a third, independent case (`run3_3.mac`'s Po-218 peak, not used in the fit): calibrated to within 2.3 keV of the true 6.0023 MeV. See the README's "PIPS dead-layer energy calibration" section for the full derivation. This only rescales the display channel axis — the raw Geant4 deposit and `Total energy` remain unaffected. Note: this branch's 200-day decay threshold means Am-241 itself won't decay here (see Fixed bug note in feature branches) — the calibration was validated with Po-218 on this branch.
