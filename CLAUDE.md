...
**PIPS dead-layer energy calibration:** the combined effect of Fe self-absorption (Am-241 source encapsulation), the Si dead layer, and the region-cut boundary above shifts the raw PIPS peak substantially below the true alpha energy (Am-241: 5.486 MeV true vs. 4.8633 MeV raw measured with `run3.mac`, ~11% low). `csv_to_dat()` in `g4decay.cc` applies a linear calibration (`PIPS_calib_slope`/`PIPS_calib_offset`, `E_true = slope*E_meas + offset`) to the PIPS channel before binning, fit from two known lines (Am-241 and Po-218, both via `run3.mac`) and cross-validated against a third, independent case (`run3_3.mac`'s Po-218 peak, not used in the fit: calibrated to within 2.3 keV of the true 6.0023 MeV). See the README's "PIPS dead-layer energy calibration" section for the full derivation. This only rescales the display channel axis — `E_pips`/`Total energy (PIPS)` remain the true raw Geant4 deposit, unaffected.
